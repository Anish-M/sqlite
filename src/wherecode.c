...
)
{
...
#endif
  {
    ...
    isSearch = (flags & (WHERE_BTM_LIMIT | WHERE_TOP_LIMIT)) != 0 || ((flags & WHERE_VIRTUALTABLE) == 0 && (pLoop->u.btree.nEq > 0)) || (wctrlFlags & (WHERE_ORDERBY_MIN | WHERE_ORDERBY_MAX));
...
static void disableTerm(WhereLevel *pLevel, WhereTerm *pTerm)
{
  int nLoop = 0;
  assert(pTerm != 0);
  while ((pTerm->wtFlags & TERM_CODED) == 0 && (pLevel->iLeftJoin == 0 || ExprHasProperty(pTerm->pExpr, EP_OuterON)) && (pLevel->notReady & pTerm->prereqAll) == 0)
  {
...
)
{
  ...
  else
  {
    ...
    if ((pLoop->wsFlags & WHERE_VIRTUALTABLE) == 0 && pLoop->u.btree.pIndex != 0 && pLoop->u.btree.pIndex->aSortOrder[iEq])
    {
      testcase(iEq == 0);
      testcase(bRev);
      bRev = !bRev;
    ...
    if (pIn)
    {
      ...
      testcase(iEq > 0 && (pLoop->wsFlags & WHERE_IN_SEEKSCAN) == 0 && (pLoop->wsFlags & WHERE_VIRTUALTABLE) != 0);
      if (iEq > 0 && (pLoop->wsFlags & (WHERE_IN_SEEKSCAN | WHERE_VIRTUALTABLE)) == 0)
      {
  ...
  */
  if ((pLevel->pWLoop->wsFlags & WHERE_TRANSCONS) == 0 || (pTerm->eOperator & WO_EQUIV) == 0)
  {
    disableTerm(pLevel, pTerm);
  }
...
)
{
  if (pTerm->wtFlags & TERM_LIKEOPT)
  {
    ...
    assert(pOp != 0);
    assert(pOp->opcode == OP_String8 || pTerm->pWC->pWInfo->pParse->db->mallocFailed);
...
static int codeCursorHintCheckExpr(Walker *pWalker, Expr *pExpr)
{
  ...
  assert(pHint->pIdx != 0);
  if (pExpr->op == TK_COLUMN && pExpr->iTable == pHint->iTabCur && sqlite3TableColumnToIndex(pHint->pIdx, pExpr->iColumn) < 0)
  {
    pWalker->eCode = 1;
  }
  return WRC_Continue;
...
{
  if (pExpr->op == TK_IS || pExpr->op == TK_ISNULL || pExpr->op == TK_ISNOT || pExpr->op == TK_NOTNULL || pExpr->op == TK_CASE)
  {
    pWalker->eCode = 1;
  }
...
)
{
  ...
  for (i = 0; i < pWC->nBase; i++)
  {
    ...
    */
    ...
    {
      Expr *pExpr = pTerm->pExpr;
      if (!ExprHasProperty(pExpr, EP_OuterON) || pExpr->w.iJoin != pTabItem->iCursor)
      {
        sWalker.eCode = 0;
...
)
{
  ...
  if ((pWInfo->wctrlFlags & (WHERE_OR_SUBCLAUSE | WHERE_RIGHT_JOIN)) && DbMaskAllZero(sqlite3ParseToplevel(pParse)->writeMask))
  {
    int i;
...
)
{
  while (++iLevel < pWInfo->nLevel)
  {
    ...
    **  vvvvv--'    pLevel->regFilter if this were true. */
    if (NEVER(pLoop->prereq & notReady))
      continue;
...
)
{
  ...
  */
  assert((pWInfo->wctrlFlags & (WHERE_OR_SUBCLAUSE | WHERE_RIGHT_JOIN)) || pLevel->iFrom > 0 || (pTabItem[0].fg.jointype & JT_LEFT) == 0);
      ...
      if ((pLoop->wsFlags & WHERE_VIRTUALTABLE) != 0)
  {
    ...
    ** the u.vtab.idxStr.  NULL it out to prevent a use-after-free */
    ...
    for (j = 0; j < nConstraint; j++)
    {
      ...
      if ((pTerm->eOperator & WO_IN) != 0 && (SMASKBIT32(j) & pLoop->u.vtab.mHandleIn) == 0 && !db->mallocFailed)
      {
        ...
        */
        for (iIn = 0; ALWAYS(iIn < pLevel->u.in.nIn); iIn++)
        {
          ...
          if ((pOp->opcode == OP_Column && pOp->p3 == iReg + j + 2) || (pOp->opcode == OP_Rowid && pOp->p2 == iReg + j + 2))
          {
    ...
    */
    ...
    if ((pLoop->wsFlags & WHERE_IPK) != 0 && (pLoop->wsFlags & (WHERE_COLUMN_IN | WHERE_COLUMN_EQ)) != 0)
    {
    ...
    else if ((pLoop->wsFlags & WHERE_IPK) != 0 && (pLoop->wsFlags & WHERE_COLUMN_RANGE) != 0)
    {
      ...
      if (pEnd)
      {
        ...
        if (0 == sqlite3ExprIsVector(pX->pRight) && (pX->op == TK_LT || pX->op == TK_GT))
        {
    ...
    else if (pLoop->wsFlags & WHERE_INDEXED)
    {
      ...
      */
      if ((pLoop->wsFlags & (WHERE_TOP_LIMIT | WHERE_BTM_LIMIT)) == 0 && (pLoop->wsFlags & WHERE_BIGNULL_SORT) != 0)
      {
      ...
      */
      ...
      if (pRangeStart)
      {
        ...
        if ((pRangeStart->wtFlags & TERM_VNULL) == 0 && sqlite3ExprCanBeNull(pRight))
        {
      ...
      */
      ...
      if (pRangeEnd)
      {
        ...
        if ((pRangeEnd->wtFlags & TERM_VNULL) == 0 && sqlite3ExprCanBeNull(pRight))
        {
          sqlite3VdbeAddOp2(v, OP_IsNull, regBase + nEq, addrNxt);
          VdbeCoverage(v);
        }
      ...
      omitTable = (pLoop->wsFlags & WHERE_IDX_ONLY) != 0 && (pWInfo->wctrlFlags & (WHERE_OR_SUBCLAUSE | WHERE_RIGHT_JOIN)) == 0;
      if (omitTable)
      {
      ...
      */
      ...
      for (ii = 0; ii < pOrWc->nTerm; ii++)
      {
        ...
        if (pOrTerm->leftCursor == iCur || (pOrTerm->eOperator & WO_AND) != 0)
        {
          ...
          testcase((pTabItem[0].fg.jointype & JT_LEFT) != 0 && !ExprHasProperty(pOrExpr, EP_OuterON)); /* See TH3 vtab25.400 and ticket 614b25314c766238 */
          ...
          if (pSubWInfo)
          {
            ...
            */
            ...
            if ((pSubLoop->wsFlags & WHERE_INDEXED) != 0 && (ii == 0 || pSubLoop->u.btree.pIndex == pCov) && (HasRowid(pTab) || !IsPrimaryKeyIndex(pSubLoop->u.btree.pIndex)))
            {
  ...
  */
  ...
  do
  {
    ...
    for (pTerm = pWC->a, j = pWC->nTerm; j > 0; j--, pTerm++)
    {
      ...
      {
        testcase(pWInfo->untestedTerms == 0 && (pWInfo->wctrlFlags & WHERE_OR_SUBCLAUSE) != 0);
      ...
      if (pTabItem->fg.jointype & (JT_LEFT | JT_LTORJ | JT_RIGHT))
      {
        ...
        else if ((pTabItem->fg.jointype & JT_LEFT) == JT_LEFT && !ExprHasProperty(pE, EP_OuterON))
        {
          continue;
        }
        else
  ...
  */
  for (pTerm = pWC->a, j = pWC->nBase; j > 0; j--, pTerm++)
  {
    ...
    if (pAlt->wtFlags & (TERM_CODED))
      continue;
    if ((pAlt->eOperator & WO_IN) && ExprUseXSelect(pAlt->pExpr) && (pAlt->pExpr->x.pSelect->pEList->nExpr > 1))
    {
      continue;
    }
  ...
  */
  ...
  code_outer_join_constraints:
    for (pTerm = pWC->a, j = 0; j < pWC->nBase; j++, pTerm++)
    {
      ...
      if (pTabItem->fg.jointype & JT_LTORJ)
        continue;
      assert(pTerm->pExpr);
  ...
  /* Inside sqlite3WhereCodeOneLoopStart(), after match check */
  if (pLevel->iLeftJoin && OptimizationEnabled(db, SQLITE_TTJ))
  {
    int regMatch = sqlite3GetTempReg(pParse);
    int addrDelete;
    int iCur = pLevel->iTabCur; /* Get the cursor for current table */

    /* Get table reference correctly */
    SrcList *pSrc = pWInfo->pTabList;
    Table *pTab = pSrc->a[pLevel->iFrom].pTab;

    /* Check if match found in hash table */
    sqlite3VdbeAddOp3(v, OP_Found, pLevel->ttjHashTable, addrCont, regMatch);

    /* If no match, prepare for deletion */
    addrDelete = sqlite3VdbeAddOp2(v, OP_IfNot, regMatch, 0);

    /* Delete the tuple from hash table */
    int regKey = sqlite3GetTempReg(pParse);
    sqlite3ExprCodeGetColumnOfTable(v, pTab, iCur, -1, regKey);
    sqlite3VdbeAddOp2(v, OP_Delete, pLevel->ttjHashTable, regKey);
    sqlite3ReleaseTempReg(pParse, regKey);

    /* Jump to backtracking if needed */
    sqlite3VdbeAddOp2(v, OP_Goto, 0, pLevel->ttjBackjumpAddr);
    sqlite3VdbeJumpHere(v, addrDelete);

    sqlite3ReleaseTempReg(pParse, regMatch);
  }
    ...
    WhereLevel *pLevel)
{
  ...
  if ((pTabItem->fg.jointype & JT_LTORJ) == 0)
  {
    ...
    for (k = 0; k < pWC->nTerm; k++)
    {
      ...
      if ((pTerm->wtFlags & (TERM_VIRTUAL | TERM_SLICE)) != 0 && pTerm->eOperator != WO_ROWVAL)
      {
        break;
      }
...
void whereDoTTJBackjump(Parse *pParse, WhereInfo *pWInfo, WhereLevel *pLevel)
{
  ...
  {
    /* At parent relation - delete tuple */
    sqlite3VdbeAddOp1(v, OP_Delete, pLevel->ttjHashTable);
    /* Reset match count */
    sqlite3VdbeAddOp2(v, OP_Integer, 0, pLevel->regMatchCount);
  }
  else
  {
    /* Clear matches and propagate backjump */
    sqlite3VdbeAddOp2(v, OP_Null, 0, pLevel->regMatches);

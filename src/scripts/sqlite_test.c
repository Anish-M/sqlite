...
#define NUM_JOIN_DEPTHS 4  // Join depth from 10 to 300 (increment by 10)
#define NUM_TESTS_PER_JOIN_DEPTH 3
...
void insert_data(sqlite3 *db, int table_num) {
    for (int i = 0; i < NUM_ROWS; i++) {
        ...
        for (int j = 0; j < NUM_COLUMNS; j++) {
            ...
            if (j % 4 == 0) {
                snprintf(query + strlen(query), sizeof(query) - strlen(query), "%d", rand() % 100);
            } else if (j % 4 == 1) {
                snprintf(query + strlen(query), sizeof(query) - strlen(query), "'%c'", (char)(rand() % 26 + 'A'));
            } else if (j % 4 == 2) {
                snprintf(query + strlen(query), sizeof(query) - strlen(query), "'%s'", "test");
            } else {
...
void create_query(int join_depth, const char* join_column, char *query) {
    ...
    for (int i = 2; i <= join_depth; i++) {
        snprintf(query + strlen(query), 4096 - strlen(query), " JOIN table%d ON table1.%s = table%d.%s", i, join_column, i, join_column);
    }
}
    ...
    for (int join_depth = 2; join_depth <= NUM_JOIN_DEPTHS; join_depth += 1) {
        ...
        for (int column_idx = 0; column_idx < NUM_COLUMNS; column_idx++) {
            ...
            for (int test_num = 0; test_num < NUM_TESTS_PER_JOIN_DEPTH; test_num++) {
                ...
                total_time += execute_query(db, query, logfile);
            }
            ...
            store_query_time(db, join_depth, join_column, average_time, query);

            printf("Average time for join depth %d, column %s: %f seconds.\n------------------------------------------\n\n", join_depth, join_column, average_time);
            fprintf(logfile, "Average execution time for join depth %d, column %s: %f seconds.\n------------------------------------------\n\n", join_depth, join_column, average_time);
        }
    }
